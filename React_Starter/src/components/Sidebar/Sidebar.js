...
import org_nav from "./_nav";
import SidebarFooter from "./../SidebarFooter";
import SidebarForm from "./../SidebarForm";
import SidebarHeader from "./../SidebarHeader";
import SidebarMinimizer from "./../SidebarMinimizer";

let nav = Object.create(org_nav);
...
class Sidebar extends Component {
  ...
  componentWillMount() {
    //change sidebar
    nav = Object.create(org_nav);
    console.log(localStorage.getItem("login_session"));

    if (localStorage.getItem("login_session")) {
      let user_role = JSON.parse(localStorage.getItem("login_session")).role;
      let arr = [];
      if (user_role == 10) {
        //nav.items.shift();
        nav.items.map(i => {
  ...
  render() {
    ...
    const navType = (item, idx) =>
      item.title
        ? title(item, idx)
        : item.divider ? divider(item, idx) : item.label ? navLabel(item, idx) : item.children ? navDropdown(item, idx) : navItem(item, idx);
